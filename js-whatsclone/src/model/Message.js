...
export class Message extends Model {
    ...
    /** Método que pelo tipo de mensagem envia um corpo 
        @me atributo por padrão true indica que o usuario que enviou a mensagem, false foi o contato
    */
    getViewElement(me = true) {
        ...
        switch(this.type) {
            ...
            break;
            case 'document':
                if(this.fileType && this.preview) div.innerHTML = `
    ...
    /** Método que retorna o status da mensagem */
    ...
    /** Método que envia a mensagem para o contato, definido pelo chatId 
        @chatId identificador do chat entre usuario e contato
        @from quem enviou a mensagem
        @type o tipo de mensagem
    ...
    */
        ...
        return new Promise((s, f) => {
            Message.getRef(chatId).add({
                ...
                docRef.set({
                    ...
                    merge: true // mantem os outros dados intactos
                }).then(() => {
                    s(docRef);
                });
            });
    ...
    /** Método que envia um documento para o contato, definido pelo chatId 
        @chatId identificador do chat entre usuario e contato
        @from quem enviou a mensagem
        @file o arquivo de audio gerado
        @metadata informações do arquivo de audio
        @photo foto do perfil de quem enviou o audio
    */
    static sendAudio(chatId, from, file, metadata, photo) {
        return Message.send(chatId, from, 'audio', '').then(msgRef => {
            Message.upload(file, from).then(snapshot => {
                let downloadAudioFile = snapshot;
                msgRef.set({
                    content: downloadAudioFile,
                    size: file.size,
                    fileType: file.type,
                    status: 'sent',
                    photo,
    ...
    /** Método que envia um contato via chat, definido pelo chatId 
        @chatId identificador do chat entre usuario e contato
        @from quem enviou a mensagem
        @contact contato que será enviado
    */
    ...
    */
    ...
    /** Método que envia uma imagem para o contato, definido pelo chatId 
        @chatId identificador do chat entre usuario e contato
        @from quem enviou a mensagem
    ...
    */
    static sendImage(chatId, from, file) {
        return new Promise((s, f) => {
            Message.upload(file, from).then(() => {
                Message.send(chatId, from, 'image', snpashot.downloadURL).then(() => {
                    s();
                });
            });
        });
    ...
    /** Pega através da coleção de chats a referencia para as mensagens passadas pelo chatId
        @chatId identificador unico de chat entre usuario e contato
    */
    static getRef(chatId) {
        return Firebase.db().collection('chats').doc(chatId).collection('messages');
    }

    /** Método que faz o upload dos arquivos para enviar na mensagem 
        @file arquivo que será feito o upload
        @from quem enviou a mensagem
    */
